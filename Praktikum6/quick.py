...
def partition(data,first,last):
    ...
    while not done:
        ...
        while leftmark <= rightmark and data[leftmark] >= pivotvalue: #mengubah <= pivotvalue jadi >= pivotvalue
            leftmark = leftmark + 1

        while data[rightmark] <= pivotvalue and rightmark >= leftmark:#mengubah >= pivotvalue jadi <= pivotvalue
